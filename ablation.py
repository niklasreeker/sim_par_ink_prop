# -*- coding: utf-8 -*-
"""Ablationsstudie: Wert des Schallsensors. Messmodell = validierter ink_calculator.
   Filter laeuft mit (A) Waage+Dichte+Schall vs. (B) Waage+Dichte."""
import numpy as np
...
def make_u(k): return dict(n_Donor=n_donor[k],mdot_SL=mdot_SL[k],mdot_W=mdot_W[k],T=T_true[k])

X=np.zeros((steps,NC+1)); x=np.concatenate([m0,[kp_profile[0]]])
for k in range(steps):
    x[NC]=kp_profile[k]; X[k]=x; x=process_step(x,make_u(k),dt)
w_al_true=X[:,IDX["Al"]]/X[:,:NC].sum(1)
...
def run(use_sound):
    x0=np.concatenate([m0*np.array([1.03,0.97,1.05,1.0]),[5.0]])
    P0=np.diag(np.concatenate([(0.05*m0)**2,[5.0**2]]))
    Q=np.diag(np.concatenate([(np.array([2e-4,2e-4,2e-4,5e-4]))**2,[0.02**2]]))
    ...
    for k in range(steps):
        ...
        m=avail[k].copy()
        if not use_sound: m[2]=False
        if m.any(): f.update(meas[k],u,mask=m)
        wal[k]=f.w_al(); s[k]=f.w_al_std()
    return wal,s

walA,sA=run(True); walB,sB=run(False)
...
ax[0].set_ylabel("σ(w_Al) [%-Pkt]"); ax[0].set_title("Wert des Schallsensors: Unsicherheit der Al-Schätzung (reales Kennfeld)")
